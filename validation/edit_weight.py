...
def main():
    ...
    for head_out_name, list_int_vec in interventions.items():
        ...
        displacement = np.zeros(
            (int(num_heads), int(model.config.hidden_size / num_heads))
        )
        ...
        displacement = torch.tensor(
            rearrange(displacement, "h d -> (h d)"), device=device
        )
        ...
        model.model.layers[layer_no].self_attn.o_proj.bias = (
            torch.nn.parameter.Parameter(bias_tobe)
        )
