...
def load_triviaqa():
    ...
    for row in dataset:
        new_row = pd.DataFrame(
            {
                "question": [row["question"]],
                "answer": [[_ for _ in row["answer"]["aliases"]]],
                "false_answer": [row["false_answer"]],
            }
        )
        df = pd.concat([df, new_row], ignore_index=True)
            ...
            if pd.isnull(frame.loc[idx, "{0} lprob max".format(tag)]):
                ...
                for temp_ans in ref_true:
                    ...
                    if interventions == {}:
                        ...
                        intervene = partial(
                            intervention_fn, start_edit_location=start_edit_location
                        )

                    with TraceDict(
                        model, layers_to_intervene, edit_output=intervene
                    ) as ret:
...
def run_kl_wrt_orig(
    ...
    if separate_kl_device is not None:
        orig_model = AutoModelForCausalLM.from_pretrained(
            ENGINE_MAP[model_key], torch_dtype=torch.float16, low_cpu_mem_usage=True
        )
        orig_model.to("cuda")
    ...
    with torch.no_grad():
        ...
        for i in tqdm(rand_idxs, desc="run_kl_wrt_orig"):
            ...
            if separate_kl_device is not None:
                orig_logits = (
                    orig_model(input_ids.to("cuda")).logits.cpu().type(torch.float32)
                )
            else:
            ...
            if metric == "bleurt":
                try:
                    questions = metrics.run_BLEURT(
                        model_key, questions, cache_dir=cache_dir
                    )
                    utilities.save_questions(questions, output_path)
                except Exception as err:
                    print(err)
